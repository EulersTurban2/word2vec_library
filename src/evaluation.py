...
def normalize(model:SkipGram_Model|CBOW_Model):
    embeddings = model.W_in
    norms = np.linalg.norm(embeddings, axis=1, keepdims=True)
    model.W_in = embeddings / (norms + 1e-10)

def build_normalized_embeddings(model:SkipGram_Model|CBOW_Model):
    combined = (model.W_in + model.W_out.T) / 2
    norms = np.linalg.norm(combined, axis=1, keepdims=True)
    return combined / (norms + 1e-10)
...
def get_nearest_neighbors(model:SkipGram_Model|CBOW_Model, word:str, word2idx:dict, idx2word:dict, top_k:int = 5) -> list[str]:
    ...
    normalized_embeddings = build_normalized_embeddings(model=model)
